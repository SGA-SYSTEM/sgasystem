        ...
        if not usuario_prova.has_expired():
            if not usuario_prova.tempo_inicial:
                ...
                messages.add_message(request, messages.ERROR,
                                     'Esta prova já foi inicializada!')
        else:
        ...
        if usuario_prova.get_progress() == '100%':
            if not usuario_prova.tempo_final:
                ...
                messages.add_message(
                    request, messages.SUCCESS, 'Prova enviada!')
                return HttpResponseRedirect('/prova/')
            else:
                messages.add_message(
                    request, messages.ERROR, 'Prova em andamento.')
        else:
            messages.add_message(
...
def questao(request, prova_id, questao_id):
    ...
    if request.user.id == usuario_prova.user.id:
        ...
        respostas = Resposta.objects.filter(
            questao__id=questao_id).order_by('id')
        try:
            usuario_resposta = UsuarioProvaResposta.objects.get(
                usuario_prova__id=prova_id, questao__id=questao_id)
        except:
...
def send_response(request):
    if request.method == 'POST':
        ...
        usuario_prova_resposta = UsuarioProvaResposta.objects.filter(
            usuario_prova__id=usuario_prova_id, questao__id=questao_id)
        usuario_prova_resposta.delete()

        usuario_prova_resposta = UsuarioProvaResposta(
            usuario_prova=UsuarioProva(
                id=usuario_prova_id), questao=Questao(id=questao_id),
            resposta_alternativa=Resposta(id=resposta_id))
...
def dashboard(request):
    usuario_prova_list = UsuarioProva.objects.filter(
        user__id=request.user.id).order_by('id')
    try:
        exams_pending = len(
            [x for x in usuario_prova_list if x.get_status() != 'Finalizada!'])
    except Exception:
        exams_pending = None
    context = {
        ...
        'menu_progress': UsuarioProva.objects.filter(
            user__id=request.user.id).order_by('id')[0:6]
    }
...
def overview(request):
    ...
    if len(usernames) == 5 and len(titulos) == 5:
        for titulo in titulos:
            query_one = UsuarioProva.objects.filter(
                user__username=usernames[0], prova__titulo=titulo)
            if query_one:
                ...
                lista1.append(0)
        for titulo in titulos:
            query_two = UsuarioProva.objects.filter(
                user__username=usernames[1], prova__titulo=titulo)
            if query_two:
        ...
        if usernames[2]:
            for titulo in titulos:
                query_three = UsuarioProva.objects.filter(
                    user__username=usernames[2], prova__titulo=titulo)
                if query_three:
        ...
        if usernames[3]:
            for titulo in titulos:
                query_four = UsuarioProva.objects.filter(
                    user__username=usernames[3], prova__titulo=titulo)
                if query_four:
        ...
        if usernames[4]:
            for titulo in titulos:
                query_five = UsuarioProva.objects.filter(
                    user__username=usernames[4], prova__titulo=titulo)
                if query_five:
...
def list_exam(request):
    ...
    context = {
        ...
        'count_user': Profile.objects.all().count(),
        'menu_progress': UsuarioProva.objects.filter(
            user__id=request.user.id).order_by('id')[0:10]
    }
