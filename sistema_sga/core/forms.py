...
class ProfileForm(forms.ModelForm):
    ...
    class Meta:
        model = Profile
        fields = (
            'picture', 'college', 'first_name',
            'last_name', 'username', 'email', 'social_link_fc'
        )
...
class SignupForm(forms.Form):
    college = forms.CharField(widget=forms.TextInput(
                              ...
                              max_length=100,
                              label='Universidade ou Instituição de Ensino'
                              )
...
class ContactForm(forms.ModelForm):
    business = forms.CharField(widget=forms.TextInput(
    ...
    message = forms.CharField(widget=forms.Textarea(
        attrs={'placeholder': 'Mensagem', 'rows': '4',
               'required': 'required'}), max_length=100)
